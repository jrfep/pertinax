...
```
Vamos a comparar ocho modelos para la variable Tcall_dur
...
```{r}
f000 <- lme(Tcall_dur~Region,dts,random=~1|LocCode/IndivGroup, method="ML")
```
Nulo + efecto aleatorio de isla/continente
```{r}
f010 <- lme(Tcall_dur~Region,dts,
  random=list(LocCode=pdDiag(~Region),IndivGroup=pdDiag(~Region)), method="ML")
 ```
 Nulo + heterocedasticidad
 ```{r}
f001 <- lme(Tcall_dur~Region,dts,random=~1|LocCode/IndivGroup,weights=varIdent(form=~1|Region), method="ML")
```
Nulo + efecto aleatorio de isla/continente + heterocedasticidad
 ```{r}
f011 <-
  lme(Tcall_dur~Region,dts,
...
```
 ...
 ```{r}
f100 <- lme(Tcall_dur~Region+Long,dts,random=~1|LocCode/IndivGroup, method="ML")

```
 Alternativo + efecto aleatorio de isla/continente
 ```{r}
f110 <- lme(Tcall_dur~Region+Long,dts,
      random=list(LocCode=pdDiag(~Region),IndivGroup=pdDiag(~Region)), method="ML")

      ```
Alternativo + heterocedasticidad
 ```{r}
f101 <- lme(Tcall_dur~Region+Long,dts,random=~1|LocCode/IndivGroup,weights=varIdent(form=~1|Region), method="ML")
...
El Mejor modelo incluye efectos fijos de Isla, con heterocedasticidad. El modelo con longitud en el efecto fijo tiene un soporte similar (delta AIC < 2).
