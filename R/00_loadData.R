...
workdir <- "~/tmp/pertinax"
homedir <- "~/mi.git/pertinax-clamorosa"
...
dts <- read.csv(sprintf("%s/data/mdf_JR_15viii19.csv",homedir))
...
f00 <- lme(S1_dur~Region+Long,dts,random=~1|LocCode/IndivGroup)

f10 <- lme(S1_dur~Region+Long,dts,
           random=list(LocCode=pdDiag(~Region-1),IndivGroup=pdDiag(~Region-1)))
f20 <- lme(S1_dur~Region+Long,dts,
           random=list(LocCode=pdDiag(~Region),IndivGroup=pdDiag(~Region)))

f01 <- lme(S1_dur~Region+Long,dts,random=~1|LocCode/IndivGroup,weights=varIdent(form=~1|Region))
f11 <-
...
dts <- data.frame(s1,lvn.s1,loc,grp,isla,costa,long,isln)

rm(loc,grp,isla,costa,long,lvn.s1,isln,s1)

rownames(dts) <- LLC1$ID_LLAMADA
...
#############
gr.slc <- names(table(dts$grp))[table(dts$grp)>2]

sf01 <- lme(s1~isla+long,dts,
            random=~1|loc/grp,
            weights=varIdent(form=~1|isla),
            subset=grp %in% gr.slc)
sf11 <- lme(s1~isla+long,dts,
